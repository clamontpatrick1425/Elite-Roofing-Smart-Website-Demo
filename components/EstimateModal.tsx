...
const EstimateModal: React.FC<EstimateModalProps> = ({ isOpen, onClose, onScheduleClick }) => {
  if (!isOpen) {
    return null;
  }
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={onClose}>
      ...
      >
        <header className="p-5 border-b border-gray-200 flex justify-between items-center flex-shrink-0">
          <h2 className="text-2xl font-bold text-gray-900">Instant AI-Powered Estimate</h2>
        ...
        <main className="flex-1 overflow-y-auto p-4 md:p-8">
          <EstimateCalculator onScheduleClick={onScheduleClick} />
        </main>
      </div>
      <style>{`
